...
class PaymentCommandManager:
    ...
    async def handle_command(self, command: CommandMessage):
        ...
        if command.command_type == "charge_payment":
            event_type = self._charge_event_type(command.message_id)
        elif command.command_type in {"refund_payment", "cancel_payment"}:
    ...
    def _charge_event_type(self, message_id: uuid.UUID) -> str:
